...
import { AlertTriangle, CheckCircle2, Database, FileWarning, Shield, ShieldCheck, Trash2, Users, XCircle } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { formatDate } from "@/lib/format";
import { collecterStatsCRM, nettoyerDocumentsOrphelinsAction, declencherSauvegardeAction, genererRapportMensuel } from "@/lib/actions/maintenance";
    ...
    conformite.chantiersAnalyses > 0
      ? Math.round(
          ((conformite.chantiersAnalyses - conformite.chantiersNonConformes.length) /
            conformite.chantiersAnalyses) * 100
        )
      : 100;
  ...
  return (
    <div className="flex flex-col gap-6 print:gap-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        {/* Toolbar client (plein écran, enregistrer, imprimer, PDF, email) */}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className={`rounded-xl border p-4 shadow-sm ${stats.stats.documents.doublons > 0 ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Doublons fichiers</p>
          <p className={`mt-1 text-2xl font-bold ${stats.stats.documents.doublons > 0 ? "text-amber-700" : "text-emerald-700"}`}>
            {stats.stats.documents.doublons}
          </p>
        </div>
        <div className={`rounded-xl border p-4 shadow-sm ${conformite.chantiersNonConformes.length > 0 ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Non-conformités</p>
          <p className={`mt-1 text-2xl font-bold ${conformite.chantiersNonConformes.length > 0 ? "text-red-700" : "text-emerald-700"}`}>
            {conformite.chantiersNonConformes.length}
          </p>
        </div>
        <div className={`rounded-xl border p-4 shadow-sm ${stats.stats.factures.enRetard > 0 ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Factures en retard</p>
          <p className={`mt-1 text-2xl font-bold ${stats.stats.factures.enRetard > 0 ? "text-red-700" : "text-emerald-700"}`}>
            {stats.stats.factures.enRetard}
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Score processus</p>
          <p className={`mt-1 text-2xl font-bold ${scoreConformite >= 80 ? "text-emerald-600" : scoreConformite >= 60 ? "text-amber-600" : "text-red-600"}`}>
            {scoreConformite} %
          </p>
        </div>
      </div>
      ...
      {/* Navigation onglets */}
      <div className="flex gap-1 rounded-xl border border-slate-200 bg-slate-100 p-1 print:hidden">
        {ONGLETS.map((o) => {
          const Icon = o.icon;
          return (
            <Link
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition sm:text-sm ${
                onglet === o.id ? "bg-white text-brand-navy shadow-sm" : "text-slate-500 hover:text-slate-700"
              }`}
            >
        ...
        <div className="flex flex-col gap-4">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">Documents sans lien vers un client, chantier, devis ou facture</p>
              </div>
              {stats.stats.documents.orphelins === 0 ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <form action={nettoyerDocumentsOrphelinsAction}>
                    <button
                      type="submit"
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 text-center">
              {[
                { label: "Documents total", val: stats.stats.documents.total, cls: "text-brand-navy" },
                { label: "Orphelins",       val: stats.stats.documents.orphelins, cls: "text-brand-navy" },
                { label: "Doublons",        val: stats.stats.documents.doublons, cls: "text-brand-navy" },
                { label: "Valides",         val: Math.max(0, stats.stats.documents.total - stats.stats.documents.orphelins - stats.stats.documents.doublons), cls: "text-emerald-600" },
              ].map(({ label, val, cls }) => (
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="flex flex-col gap-2">
              {[
                { ok: stats.stats.factures.enRetard === 0,   msg: `${stats.stats.factures.enRetard} facture(s) en retard de paiement`, okMsg: "Aucune facture en retard" },
                { ok: stats.stats.taches.enRetard === 0,     msg: `${stats.stats.taches.enRetard} tâche(s) échue(s) non résolue(s)`,   okMsg: "Aucune tâche échue" },
                { ok: stats.stats.documents.orphelins === 0, msg: `${stats.stats.documents.orphelins} document(s) orphelin(s)`,         okMsg: "Aucun document orphelin" },
                { ok: stats.stats.documents.doublons === 0,  msg: `${stats.stats.documents.doublons} doublon(s) détecté(s)`,            okMsg: "Aucun doublon de fichier" },
              ].map((item, i) => (
                <div key={i} className={`flex items-center gap-3 rounded-lg px-4 py-3 text-sm ${item.ok ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
                  {item.ok ? <ShieldCheck className="h-4 w-4 shrink-0" /> : <XCircle className="h-4 w-4 shrink-0" />}
      ...
      {onglet === "processus" && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4 rounded-xl border border-brand-blue/20 bg-brand-blue/5 px-5 py-4">
            <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-lg font-bold text-white ${scoreConformite >= 80 ? "bg-emerald-500" : scoreConformite >= 60 ? "bg-amber-500" : "bg-red-500"}`}>
              {scoreConformite}%
            </div>
            <div>
              <p className="font-semibold text-brand-navy">Score de conformité des processus</p>
              <p className="text-sm text-slate-500">
                {conformite.chantiersAnalyses - conformite.chantiersNonConformes.length} / {conformite.chantiersAnalyses} chantiers conformes
              </p>
            </div>
          </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {conformite.chantiersNonConformes.length === 0 ? (
              <div className="flex items-center gap-3 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
                <CheckCircle2 className="h-4 w-4" /> Tous les chantiers en cours respectent les processus établis.
              </div>
            ) : (
                ...
                {conformite.chantiersNonConformes.map((c, i) => (
                  <div key={i} className="rounded-lg border border-red-100 bg-red-50 p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <span className="text-xs text-red-600 bg-red-100 px-2 py-0.5 rounded-full">{c.statut}</span>
                    </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50 text-slate-600">
                  {[
                    ...
                    ["5. Situations avancement", "Comptable",            "Avancement >30%",       "Facture de situation"],
        ...
        <div className="flex flex-col gap-4">
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mb-4 text-xs text-slate-400">
              Basé sur les tâches internes. Pour une analyse complète, générez le rapport mensuel Alba-Ayla via "Enregistrer rapport".
            </p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-xs font-semibold uppercase tracking-wider text-slate-400">
                    ...
                    <th className="pb-2 text-right">Créées</th>
                  ...
                  {activiteParUser.map((u) => {
                    ...
                    return (
                      <tr key={u.id}>
                        ...
                        <td className="py-2 text-slate-500">{ROLE_LABELS[u.role] ?? u.role}</td>
                        <td className="py-2 text-right">{u.tachesCrees}</td>
                        <td className="py-2 text-right font-medium text-emerald-600">{u.tachesTerminees}</td>
                        <td className="py-2 text-right text-amber-600">{u.tachesEnCours}</td>
                        <td className="py-2 text-right font-medium text-red-600">{u.tachesEnRetard}</td>
                        <td className="py-2 text-right">
                          <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${taux >= 80 ? "bg-emerald-100 text-emerald-700" : taux >= 50 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
                            {taux}%
                          </span>
                        </td>
                      </tr>
              ...
              {[
                ...
                <div key={i} className="rounded-lg bg-slate-50 p-3 text-center">
                  <p className="text-xl font-bold text-brand-navy">
                    {item.val}
                    {item.total !== undefined && <span className="text-sm font-normal text-slate-400">/{item.total}</span>}
                  </p>
      ...
      {onglet === "sauvegardes" && (
        <div className="flex flex-col gap-4">
          <div className={`flex items-center justify-between rounded-xl border p-5 ${!derniereSauvegarde ? "border-amber-200 bg-amber-50" : derniereSauvegarde.statut === "OK" ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}>
            <div className="flex items-center gap-3">
              {!derniereSauvegarde ? (
              ...
              <div>
                <p className="font-semibold text-slate-700">
                  {!derniereSauvegarde ? "Aucune sauvegarde effectuée" : `Dernière sauvegarde : ${formatDate(derniereSauvegarde.date)}`}
                </p>
                {derniereSauvegarde && (
                  <p className="text-xs text-slate-500">{derniereSauvegarde.tailleMo} Mo · {derniereSauvegarde.destination.split(/[/\\]/).pop()}</p>
                )}
              </div>
            </div>
            <form action={declencherSauvegardeAction}>
              <button type="submit" className="rounded-xl bg-brand-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-blue-dark">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="rounded-lg bg-slate-50 p-4 text-sm text-slate-600">
              <p className="font-medium text-brand-navy mb-2">📁 Votre base de données est hébergée sur Neon (PostgreSQL cloud)</p>
              <p className="mb-3 text-xs text-slate-500">
                Le CRM est déployé sur Vercel avec une base PostgreSQL Neon. Les données sont automatiquement répliquées par Neon. La sauvegarde manuelle ci-dessus enregistre un snapshot daté dans le journal.
              </p>
            </div>
          </section>
...
async function SauvegardeHistorique() {
  const logs = await prisma.sauvegardeLog.findMany({ orderBy: { date: "desc" }, take: 20 });
        ...
        <tbody className="divide-y divide-slate-50">
          {logs.map((log) => (
            <tr key={log.id}>
              ...
              <td className="py-2"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs">{log.type}</span></td>
              <td className="py-2 font-mono text-xs text-slate-500">{log.destination.split(/[/\\]/).pop()}</td>
              <td className="py-2 text-right text-slate-500">{log.tailleMo != null ? `${log.tailleMo} Mo` : "—"}</td>
              <td className="py-2 text-right">
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${log.statut === "OK" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
                  {log.statut}
                </span>
              </td>
            </tr>
