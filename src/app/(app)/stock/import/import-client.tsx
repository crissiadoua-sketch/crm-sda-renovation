...
const CORPS_ETAT_OPTIONS = [
  "GO","CHA","COU","ETA","MEX","MIN","PLA","ISO","CAR","PEI","PLO","ELE","CVC","VRD","DEM","BUR","GEN"
];
...
type ArticleRow = ImportArticle & { selected: boolean; key: string };
    ...
    <div className="flex flex-col gap-5">
      ...
      {!articles.length && !analysing && (
        ...
        >
          ...
          <p className="mt-1 text-sm text-slate-400">PDF, JPG, PNG, WEBP — l&apos;IA extrait automatiquement les articles</p>
      ...
      {analysing && (
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-slate-200 bg-white py-16">
          ...
          <p className="text-sm font-medium text-slate-600">Analyse en cours par l&apos;IA…</p>
          <p className="text-xs text-slate-400">Extraction des désignations, références et prix</p>
        </div>
      )}
      ...
      {error && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {error}
          <button onClick={() => { setError(null); inputRef.current?.click(); }} className="ml-3 underline">Réessayer</button>
        </div>
      )}
      ...
      {articles.length > 0 && !result && (
        <>
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400">{selectedCount} sélectionné{selectedCount > 1 ? "s" : ""} pour import</p>
              </div>
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <select value={fournisseurId} onChange={(e) => setFournisseurId(e.target.value)}
                  className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-blue/30">
                ...
                <button onClick={() => { setArticles([]); setError(null); }} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs hover:bg-slate-50">
                  Nouvelle analyse
                </button>
              </div>
            </div>
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                    ...
                    <th className="px-2 py-2 w-28">Référence fourn.</th>
                    <th className="px-2 py-2 w-16">Unité</th>
                    <th className="px-2 py-2 w-28">Prix HT</th>
                    <th className="px-2 py-2 w-24">Corps d'état</th>
                    <th className="px-2 py-2 w-28">Catégorie</th>
                    <th className="px-2 py-2 min-w-[140px]">Conditionnement</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {articles.map((row) => (
                    <tr key={row.key} className={row.selected ? "bg-white" : "bg-slate-50 opacity-50"}>
                      ...
                      <td className="px-2 py-2">
                        <input value={row.conditionnement} onChange={(e) => updateRow(row.key, { conditionnement: e.target.value })}
                          className="w-full rounded border border-transparent px-1 py-0.5 hover:border-slate-200 focus:border-brand-blue focus:outline-none" />
                      </td>
                    </tr>
                  ))}
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => { setArticles([]); setError(null); }}
