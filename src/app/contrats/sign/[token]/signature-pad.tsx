...
import { signerContrat } from "@/lib/actions/contrats-sous-traitance";
...
export default function SignaturePad({ token, contratNumero }: Props) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const result = await signerContrat(token, nom.trim(), imageSignature);
  ...
  if (success) {
    return (
      <div className="rounded-2xl bg-emerald-50 border border-emerald-200 px-6 py-8 text-center">
        ...
        <p className="text-emerald-700 font-bold text-xl">Contrat signé avec succès !</p>
        <p className="text-emerald-600 text-sm mt-2">
          Votre signature pour le contrat <span className="font-mono font-semibold">{contratNumero}</span> a été enregistrée.
          <br />SDA Rénovation a été notifié.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-5 flex flex-col gap-4">
        ...
        <p className="text-[10px] text-slate-400 text-center">
          En cliquant sur "Signer le contrat", vous acceptez les conditions et apposez votre signature électronique
