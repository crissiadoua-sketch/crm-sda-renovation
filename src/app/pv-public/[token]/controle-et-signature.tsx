...
import { signerAvecControle } from "@/lib/actions/pv-reception";
...
interface Props {
  ...
  defaultNom?: string;
  lignes: Ligne[];
  defaultResultat?: string | null;
}

type Mode = "draw" | "upload";

const CONFORMITE_OPTIONS = [
  { value: "CONFORME",     label: "✓ Conforme",     cls: "border-green-300 text-green-700 bg-green-50", activeCls: "border-green-500 bg-green-100 ring-2 ring-green-400 text-green-800" },
  { value: "NON_CONFORME", label: "✗ Non conforme", cls: "border-red-300 text-red-700 bg-red-50",      activeCls: "border-red-500 bg-red-100 ring-2 ring-red-400 text-red-800" },
  { value: "SANS_OBJET",   label: "— Sans objet",   cls: "border-slate-200 text-slate-500 bg-slate-50", activeCls: "border-slate-400 bg-slate-100 ring-2 ring-slate-300 text-slate-700" },
];

const RESULTAT_OPTIONS = [
  { value: "ACCEPTE",          label: "Réception prononcée sans réserve",  icon: "✅", cls: "border-green-200 bg-green-50 hover:bg-green-100",  activeCls: "border-green-500 bg-green-100 ring-2 ring-green-400", textCls: "text-green-800" },
  { value: "ACCEPTE_RESERVES", label: "Réception prononcée avec réserves", icon: "⚠️", cls: "border-amber-200 bg-amber-50 hover:bg-amber-100",  activeCls: "border-amber-500 bg-amber-100 ring-2 ring-amber-400", textCls: "text-amber-800" },
  { value: "REFUSE",           label: "Réception refusée",                 icon: "❌", cls: "border-red-200 bg-red-50 hover:bg-red-100",         activeCls: "border-red-500 bg-red-100 ring-2 ring-red-400",     textCls: "text-red-800"   },
];

export function ControleEtSignature({
  token,
  pvNumero,
  role,
  roleLabel,
  ...
  const [lignes, setLignes] = useState(
    lignesInitiales.map((l) => ({ id: l.id, designation: l.designation, conformite: l.conformite, observations: l.observations ?? "" }))
  );
  const [resultat,        setResultat]        = useState(defaultResultat ?? "");
  ...
  async function handleSubmit() {
    if (!resultat)      { setError("Veuillez sélectionner votre décision de réception."); return; }
    ...
    setError(null);
    try {
      const result = await signerAvecControle(
        token,
        role,
        nom.trim(),
        canvas.toDataURL("image/png"),
        lignes.map((l) => ({ id: l.id, conformite: l.conformite, observations: l.observations })),
        resultat,
      );
      if (result.ok) {
        setSuccess(true);
  ...
  if (success) {
    return (
      <div className="rounded-2xl bg-blue-50 border border-blue-200 px-6 py-8 text-center">
        ...
        <p className="text-blue-700 font-bold text-xl">Contrôle et signature enregistrés !</p>
        <p className="text-blue-600 text-sm mt-2 max-w-sm mx-auto">
          Votre évaluation et votre signature pour le PV{" "}
          <span className="font-mono font-semibold">{pvNumero}</span> ont bien été reçues.
          <br /><br />
          <strong>SDA Rénovation doit maintenant apposer sa propre signature.</strong>{" "}
          Vous recevrez un email dès que le PV sera disponible au téléchargement.
        </p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="bg-[#1E2F6E]/5 px-5 py-3 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Pour chaque livrable, indiquez le résultat constaté et vos éventuelles observations.
            </p>
          </div>
            ...
            {lignes.map((l, i) => (
              <div key={l.id} className="px-4 py-4">
                ...
                <div className="flex gap-2 flex-wrap mb-2.5">
                  {CONFORMITE_OPTIONS.map((opt) => (
                    <button
                      ...
                      onClick={() =>
                        setLignes((prev) =>
                          prev.map((x) => x.id === l.id ? { ...x, conformite: opt.value } : x)
                        )
                      }
                      className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                        l.conformite === opt.value ? opt.activeCls : opt.cls
                      }`}
                    >
                      {opt.label}
                    </button>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setLignes((prev) =>
                      prev.map((x) => x.id === l.id ? { ...x, observations: e.target.value } : x)
                    )
                  }
                  placeholder="Observations / réserves (optionnel)"
                  rows={2}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-[#1E2F6E] focus:ring-2 focus:ring-[#1E2F6E]/10 outline-none transition resize-none"
                />
              </div>
            ))}
          </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-5 flex flex-col gap-4">
          ...
          <div>
              ...
              uploadedPreview ? (
                ...
                <label className="flex flex-col items-center justify-center gap-2 w-full rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 hover:border-[#1E2F6E]/40 cursor-pointer transition" style={{ height: 120 }}>
                  <span className="text-[11px] text-slate-400 text-center px-4">
                    Cliquez pour choisir une image (PNG, JPG, WebP)
                  </span>
                  <input
                    ref={fileInputRef}
                    type="file"
            ...
            {mode === "draw" && !hasSignature && (
              <p className="text-[11px] text-slate-400 mt-1 text-center">
                Signez ici en maintenant le bouton ou en posant votre doigt
              </p>
            )}
          </div>
          ...
          <button
            ...
            disabled={pending}
            className="w-full rounded-xl bg-gradient-to-r from-[#1E2F6E] to-[#1B3F94] text-white font-bold py-3.5 text-sm hover:opacity-90 transition disabled:opacity-60 disabled:cursor-not-allowed shadow-md"
          >
            {pending ? "Enregistrement en cours…" : `✍️ Valider le contrôle et signer — ${roleLabel}`}
          </button>
