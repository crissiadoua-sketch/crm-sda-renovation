...
export function SignaturePadPv({ token, pvNumero, role, roleLabel, defaultNom = "" }: Props) {
  ...
  if (success) {
    return (
      <div className="rounded-2xl bg-emerald-50 border border-emerald-200 px-6 py-8 text-center">
        ...
        <p className="text-emerald-700 font-bold text-xl">Signature enregistrée !</p>
        <p className="text-emerald-600 text-sm mt-2">
          Votre signature en tant que <strong>{roleLabel}</strong> pour le PV{" "}
          <span className="font-mono font-semibold">{pvNumero}</span> a été enregistrée.
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-5 flex flex-col gap-4">
        ...
        >
          {pending ? "Signature en cours…" : `Signer le PV — ${roleLabel}`}
        </button>
